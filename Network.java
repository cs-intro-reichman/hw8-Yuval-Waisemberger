...
public class Network {
    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    public boolean addUser(String name) {
        // Program checks either the network is full or the user is alredy in the network
        if (userCount == users.length || getUser(name) != null) {
            return false;
        }
        // Adding the User to the last array index
    ...
    public boolean addFollowee(String name1, String name2) {
        // Checking both names are valid 
        if (name1 == null || name2 == null) {
            return false;
        }
        // Checking if both names are in the network
        if (getUser(name1) == null || getUser(name2) == null) {
            return false;
        }
        // Edge case - both users are the same (you can't follow yourself)
        if (name1.equals(name2)) {
            return false;
        }
        // Checking if the user already follows this user
    ...
    public String recommendWhoToFollow(String name) {
        int maximumMutual = 0, maximumIndex = 0;
        ...
        return users[maximumIndex].getName();
    }
